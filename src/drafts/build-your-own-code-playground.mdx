...
<iframe
  ...
  style={{ width: '100%', height: 300, border: 0, background: 'white' }}
/>
...
```ts filename=utils/execute-code {7-21}
export async function transformCode(code: string) {
  if (swc === null) {
    const module = await import('@swc/wasm-web')
    await module.default()
...
For security purposes, you should further sanitize the incoming data and add an identifier to make sure you're dealing with code you trust. Now, similarly as before, we'll add a `useEffect` hook in our `index` page to send the code to the iframe:
