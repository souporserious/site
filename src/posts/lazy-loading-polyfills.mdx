...
In the last post, we looked at [implementing a dialog element in React](/build-a-dialog-component-in-react/). Let's look at how we can import the dialog polyfill lazily, so unsupported browsers still get the same experience, and we minimize the cost that supported browsers incur.
